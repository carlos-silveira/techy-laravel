...
class DeepCleanArticles extends Command
{
    ...
    private function removeDuplicates()
    {
        ...
        $imageDuplicates = DB::table('articles')
            ->select(DB::raw('SUBSTRING_INDEX(cover_image_path, "?", 1) as base_image'), DB::raw('count(*) as count'))
            ->whereNotNull('cover_image_path')
            ->where('cover_image_path', '!=', '')
            ->groupBy(DB::raw('SUBSTRING_INDEX(cover_image_path, "?", 1)'))
            ->having('count', '>', 1)
            ->get();
        ...
        if ($imageDuplicates->isNotEmpty()) {
            foreach ($imageDuplicates as $duplicate) {
                ...
                $articles = Article::whereRaw('SUBSTRING_INDEX(cover_image_path, "?", 1) = ?', [$duplicate->base_image])
                    ->orderBy('views_count', 'desc')
                    ->orderBy('created_at', 'desc')
                    ->get();
