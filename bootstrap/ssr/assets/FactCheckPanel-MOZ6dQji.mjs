...
import { a as AnimatePresence } from "./x-DrxCXmes.mjs";
import { r as ShieldCheck } from "./users-ClvImKtF.mjs";
import { t as ChevronDown } from "./chevron-down-BxBMT95t.mjs";
...
function FactCheckPanel({ articleId }) {
	...
	const fetchFactCheck = async () => {
		setIsLoading(true);
		try {
			setFactCheck((await axios.get(`/api/articles/${articleId}/fact-check`)).data);
		} catch (error) {
	...
	const triggerFactCheck = async () => {
		...
		try {
			setFactCheck((await axios.post(`/api/articles/${articleId}/fact-check`)).data.fact_check);
