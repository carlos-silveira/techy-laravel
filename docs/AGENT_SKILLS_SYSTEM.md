...
1. **Deployment Token**: The `X-Deploy-Token` is currently hardcoded in `deploy.yml`. 
    ...
    - **Action**: Move `TechyDeploySecret2026Secure` to a GitHub Repository Secret named `TECHY_DEPLOY_TOKEN` and update the workflow to user `${{ secrets.TECHY_DEPLOY_TOKEN }}`.
