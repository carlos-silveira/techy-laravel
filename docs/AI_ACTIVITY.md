...
- **Performance (Cache)**: SSH'd into the production server (`66.29.148.140`) and ran `php artisan cache:clear` to purge the bloated JSON payloads (which were cached for 1 hour). The production payload instantly dropped from ~100KB to ~23KB, eliminating the 3.1s React render delay.
...
- **Server (PageSpeed/Caching)**: Added standard `mod_expires` and `mod_deflate` directives to `public/.htaccess` to enable browser caching for static assets (images, fonts, CSS/JS) and GZIP compression. This directly addresses Google PageSpeed's warnings to "Serve static assets with an efficient cache policy" and "Enable text compression", leading to drastically lower TTFB and transfer sizes.
