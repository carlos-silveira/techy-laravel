...
export default function RagCopilot() {
    ...
    return (
        <>
            ...
            <div className="fixed bottom-6 right-3 sm:right-6 z-[100] flex flex-col items-end">
                <AnimatePresence>
                    {isOpen && (
                        <motion.div
                            ...
                            className="mb-4 fixed sm:relative bottom-[75px] sm:bottom-auto right-3 left-3 sm:right-auto sm:left-auto sm:w-[450px] h-[calc(100vh-110px)] sm:h-[650px] sm:max-h-[85vh] flex flex-col bg-white dark:bg-[#0a0f1c] sm:bg-white/95 sm:dark:bg-[#0a0f1c]/95 sm:backdrop-blur-2xl border border-black/10 dark:border-white/10 rounded-[28px] shadow-2xl overflow-hidden z-[100]"
                        >
                            {/* Header */}
                            <div className="flex items-center justify-between p-4 border-b border-black/5 dark:border-white/5 bg-gray-50/80 dark:bg-white/[0.02] backdrop-blur-md">
                                ...
                                <button onClick={() => setIsOpen(false)} className="p-2 text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors rounded-full hover:bg-black/5 dark:hover:bg-white/5 bg-transparent">
                                    <X className="w-5 h-5" />
                                </button>
                            </div>
                            ...
                            <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-5 scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-white/10">
                                {messages.map((msg, idx) => (
                                    <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 w-full`}>
                                        <div className={`max-w-[95%] sm:max-w-[85%] rounded-[24px] p-4 text-[15px] flex flex-col shadow-sm ${
                                            msg.role === 'user' 
                                                ...
                                                <div className="prose prose-sm dark:prose-invert prose-p:leading-relaxed prose-a:text-primary max-w-full break-words">
                                                    <ReactMarkdown 
                                                        ...
                                                        components={{
                                                            ...
                                                            th: ({node, ...props}) => <th className="p-3 bg-gray-50/50 dark:bg-white/[0.02] border-b border-black/10 dark:border-white/10 font-bold" {...props} />,
                                                            td: ({node, ...props}) => <td className="p-3 border-b border-black/5 dark:border-white/5 last:border-0" {...props} />,
                            ...
                            {/* Input Area */}
                            <div className="p-3 sm:p-4 bg-white dark:bg-[#0a0f1c] border-t border-black/5 dark:border-white/5">
                                <div className="relative flex items-center bg-gray-100 dark:bg-[#1a2030] rounded-full p-1.5 border border-transparent focus-within:border-primary/30 focus-within:ring-2 focus-within:ring-primary/20 transition-all">
                                    <textarea
                                        ...
                                        className="flex-1 bg-transparent text-gray-900 dark:text-white text-[15px] py-2.5 px-4 resize-none border-none focus:ring-0 placeholder:text-gray-500 overflow-hidden"
                                        rows="1"
                                        style={{ minHeight: '44px', maxHeight: '120px' }}
                                    />
                                    <div className="flex-shrink-0 pr-1">
                                        {isLoading ? (
                                            <button onClick={handleStop} className="p-2 text-white bg-red-500 hover:bg-red-600 transition-all rounded-full shadow-sm">
                                            ...
                                            <button 
                                                onClick={handleSend}
                                                disabled={!input.trim()}
                                                className="p-2 text-white bg-primary hover:bg-purple-600 disabled:bg-gray-300 dark:disabled:bg-white/10 disabled:text-gray-500 transition-all rounded-full shadow-sm"
                                            >
